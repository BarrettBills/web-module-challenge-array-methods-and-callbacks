...
function getFinals(array) {
    const finalStage = fifaData.filter((item) => {
...
function getWinnersByYear(array, cbF, cbY, cbW) {
    const theBest = cbW(array, cbF);
    const momentInTime = cbY(array, cbF); 
    return theBest.map((item, index) => `In ${momentInTime[index]}, ${item} won the world cup!`);
}
...
function getAverageGoals(cb) {
    const avgGoals = cb.reduce(function(accumulator, item){
        return accumulator + item['Home Team Goals'] + item['Away Team Goals'];
    }, 0)
    return (avgGoals / cb.length).toFixed(2);
}
